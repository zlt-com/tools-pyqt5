...
from threader.threader import (
    ParserGovHtmlSaveFileThread,
    FileConvertThread,
    ScanFileLogThread,
)
...
class Run(QtWidgets.QWidget, Ui_MainPage):
    def __init__(self):
        ...
        # 初始化扫描日志表格
        self.tab3_table_scan_files.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.tab3_table_parser_files.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
    ...
    def target_file_type_select_change(self):
        ...
        target_key = constant.get_can_convert_file_type(
            self.source_file_type_select.currentData()
        )
    ...
    def btn_select_file(self):
        ...
        root = "."
        self.selected_dir = QFileDialog.getOpenFileNames(
            self, caption="选择文件", directory=root, filter=file_type
        )
    ...
    def file_conv_select_save_directory(self):
        self.save_dir = QFileDialog.getExistingDirectory(
            self, caption="选择转换保存文件夹", directory=""
        )
    ...
    def converter(self, f, out_put_dir):
        ...
        try:
            converter = ConverterFactory(
                self.target_file_type_select.currentData()
            ).get_converter()
            if self.source_file_type_select.currentData() == "image":
            ...
            self.file_conv_result_text.setText(
                f"{log_file_name} 转换为{self.target_file_type_select.currentText()}成功。\r\n{self.file_conv_result_text.toPlainText()}"
            )
        except Exception as e:
            print(e)
    ...
    def converter_mutil_thread(self, f, out_put_dir):
        ...
        if self.source_file_type_select.currentData() == "image":
            ...
            thread = FileConvertThread(
                converter, f, out_put_dir, self.file_convert_log_text_signal
            )
    ...
    def parser_web_site_content(self):
        try:
            thread = ParserGovHtmlThread(
                self.tab2_txt_web_url.text(),
            ...
            for i in range(2, 22):
                ...
                thread = ParserGovHtmlThread(
                    url, self.parser_html_log_text_signal, self.tab2_txt_key.text()
                )
                self.pool.start(thread)
    ...
    def write_parser_html_log(self, text):
        ...
        self.tab2_text_content.setText(
        ...
        if self.tab3_checkbox_scan_all_disk.isChecked():
            ...
            for disk in disks:
                scan_thread = ScanFileLogThread(
                    disk=disk, signal=self.scan_file_log_text_signal
                )
                self.scan_threads.append(scan_thread)
        else:
            d = self.tab3_combox_select_disk.currentText()
            scan_thread = ScanFileLogThread(
                disk=d, signal=self.scan_file_log_text_signal
            )
            self.scan_threads.append(scan_thread)
    ...
    def write_scan_file_log(self, text):
        ...
        self.tab3_table_scan_files.setItem(
            row_count, 0, QtWidgets.QTableWidgetItem(text)
        )
    ...
    def write_parser_scan_file_log(self, result):
        ...
        for i in range(len(result.keys)):
            ...
            self.tab3_table_parser_files.setItem(
                row_count, 0, QtWidgets.QTableWidgetItem(result.file)
            )
            self.tab3_table_parser_files.setItem(
                row_count, 1, QtWidgets.QTableWidgetItem(result.keys[i])
            )
            self.tab3_table_parser_files.setItem(
                row_count, 2, QtWidgets.QTableWidgetItem(result.context[i])
            )
